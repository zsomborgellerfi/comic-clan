...
class ComicDetails extends React.Component {
    ...
    render() {
        ...
        const otherComics = _.shuffle(comics.filter(comic => !comic.name.includes(this.props.match.params.name)).slice(0, 5));
        console.log(otherComics)

        return (
            <Container fluid className={'ComicDetails'}>
                <div className={'ComicDetails__link'}>
                    <Link to={"/comics"}><IoIosArrowBack /><span>Back to collection</span></Link>
                </div>
                {comic && <Fragment>
                    <div className={"ComicDetails__comicWrapper"}>
                        ...
                        <div>
                            <div className={"ComicDetails__name"}>
                                <h1>{comic.name} ({comic.year})</h1>
                                <div className={"ComicDetails__ratings"}>
                                    {[...Array(comic.rating)].map(() => <img src={fullStar} alt={`${comic.rating} star`} />)}
                                    {[...Array(5 - comic.rating)].map(() => <img src={emptyStar} alt={`${comic.rating} star`} />)}
                                </div>
                            </div>
                            <div className={"ComicDetails__info"}>
                                <div>Writer: <span>{comic.writer}</span></div>
                                <div>Artist: <span>{comic.artist}</span></div>
                                <div>Publication: <span>{comic.publication}</span></div>
                                <div>Owner: <span>{comic.owner}</span> </div>
                            </div>
